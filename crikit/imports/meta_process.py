    ...
    if isinstance(rosetta[key],list):  # There is a priority list
        for num, count in enumerate(rosetta[key]):
            ...
            try:
                ...
                else:  # rosetta[key] is a value to use
                    temp_val = count
                    temp_key = key
            except:
                # If it's the last option and of type str, use the value
                # from rosetta[key] as the definitive value
                if num + 1 == len(rosetta[key]):
                    if isinstance(count,str):
                        temp_val = count
                        temp_key = key
...
if __name__ == '__main__':
    ...
    from crikit.imports.meta_configs import (special_nist_bcars2
                                            as _snb)

    from crikit.imports.hdf5 import hdf_import_data as _hdf_import_data
    rosetta = _snb()
